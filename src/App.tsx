...
export default function App() {
  ...
  function exportToCSV() {
    ...
    const rows = filteredRepairs.map(r => [
      r.wagons?.wagon_number, STATUS_RU[r.current_status] || r.current_status, r.repair_type, r.wagons?.owner,
      r.sla_deadline ? new Date(r.sla_deadline).toLocaleString() : '', r.forecast_release ? new Date(r.forecast_release).toLocaleString() : ''
    ]);
        ...
        <div className="backdrop" onClick={(e) => { if (e.target === e.currentTarget) setSelectedCase(null); }}>
          <div className="bottom-sheet">
            ...
            {isInitialPhase ? (
              ...
              <>
                {/* Блок модели времени для вагона в ремонте */}
                {selectedMetrics && (
                  <div className="premium-card">
                ...
                <div className="premium-card">
                  ...
                  {availableTransitions.length === 0 ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                      ...
                      <button className="btn-primary" style={{ padding: '6px 12px', fontSize: '11px', width: 'auto' }} onClick={() => { setDocType('Справка ВУ 36М'); document.getElementById('doc-number-input')?.focus(); }}>→ Подгрузить справку ВУ 36М</button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {availableTransitions.map((st: string) => (
                        <button key={st} disabled={loading} onClick={() => handleUpdateStatus(st)} className="btn-primary" style={{ padding: '6px 10px', fontSize: '11px', width: 'auto' }}>→ {STATUS_RU[st] || st}</button>
                      ))}
                    </div>
                  )}
                </div>
                    ...
                    {statusHistory.map((ev: any) => (
                      <div key={ev.event_id || ev.event_datetime} style={{ fontSize: '10px', padding: '6px', background: 'var(--bg-color)', borderRadius: '6px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold' }}>
                          ...
                          <span style={{ color: 'var(--brand-color)', fontWeight: 'normal' }}>👤 {ev.users?.name || 'Система'}</span>
                        </div>
        ...
        <div className="backdrop">
          <div className="bottom-sheet">
            ...
            <select className="select-field" value={delayType} onChange={e => setDelayType(e.target.value as any)}>
              <option value="PRIMARY">PRIMARY (Первичная)</option><option value="SECONDARY">SECONDARY (Вторичная)</option>
            </select>
            <select className="select-field" value={delayCategory} onChange={e => setDelayCategory(e.target.value)}>
              <option value="Materials">Материалы / Запчасти</option><option value="Customer">Заказчик / Согласование</option><option value="Railway">ЖД / Маневры</option><option value="Internal">Внутренняя / Кадры</option>
            </select>
