            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center', marginTop: '4px' }}>
                  ...
                  {!isGuest && selectedCase.current_status === CASE_STATUS.IN_REPAIR && (
                    <button 
                      className="btn-secondary" 
                      style={{ padding: '2px 8px', fontSize: '10px', color: 'var(--status-paused)', borderColor: 'var(--status-paused)', width: 'auto' }}
                      onClick={() => handleUpdateStatus(CASE_STATUS.PAUSED)}
                    >
                      ⏸ Пауза смены
                    </button>
                  )}
              ...
              <div className="premium-card">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {DEFAULT_SHOPS.map(s => {
                    ...
                    const isPausedStage = prog.status === 'PAUSED';
                    ...
                    return (
                      <div key={s.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: isInProgress ? 'var(--status-repair-bg)' : isPausedStage ? 'var(--status-paused-bg)' : 'var(--bg-main)', borderLeft: isInProgress ? '3px solid var(--brand)' : isPausedStage ? '3px solid var(--status-paused)' : 'none', padding: '6px 10px', borderRadius: '6px', fontSize: '11px' }}>
                        <div>
                          <div style={{ fontWeight: 'bold' }}>{s.label}
                            {!isNotRequired && timeInfo.text && (
                              <span style={{ color: timeInfo.isOverdue ? 'var(--status-paused)' : isInProgress ? 'var(--brand)' : 'var(--text-secondary)', fontSize: '10px', marginLeft: '4px', fontWeight: timeInfo.isOverdue ? 'bold' : 'normal' }}>
                                ({isInProgress ? 'В работе: ' : isDone ? 'Итого: ' : isPausedStage ? 'Приостановлен: ' : ''}{timeInfo.text}){timeInfo.isOverdue && ' ⚠️ Превышение!'}
                              </span>
                            )}
                          </div>
                        ...
                        <div>
                          {isDone ? (
                            ...
                            canEdit ? (
                              <div style={{ display: 'flex', gap: '4px' }}>
                                <button className="btn-primary" style={{ padding: '3px 8px', fontSize: '10px', width: 'auto', background: 'var(--status-ready)' }} onClick={() => handleUpdateShopStage(s.key, 'IN_PROGRESS')} disabled={loading}>▶ Продолжить</button>
                              </div>
                            ) : <span style={{ color: 'var(--status-paused)', fontSize: '10px', fontWeight: 'bold' }}>⏸ На паузе</span>
                          ) : isInProgress ? (
                            canEdit ? (
                              <div style={{ display: 'flex', gap: '4px' }}>
                                ...
                                <button className="btn-secondary" style={{ padding: '3px 6px', fontSize: '10px', width: 'auto', color: 'var(--status-paused)' }} onClick={() => handleUpdateShopStage(s.key, 'PAUSED')} disabled={loading}>⏸ Пауза</button>
                              </div>
